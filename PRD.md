...
| **Builder** | Claude Code (executes phases; escalates deviations via ADR) |
...
> **How to use this document:** Claude Code executes phases **sequentially** (§9). Each phase ends at its Definition of Done (DoD). Stop there, commit artifacts (code + tests + docs + build log entry), and wait for owner review. Any deviation from a default in this PRD requires an ADR file in `docs/adr/` committed in the same change. Never silently change scope, targets, or stack.
...
**Portfolio goals (equal priority to product goals):** demonstrate clinical NLP, LLM information extraction, hybrid retrieval, calibrated ML, grounded generation, text2SQL, eval-first engineering, and a fully logged decision trail (ADRs + weekly build logs).
...
Initial targets — revise only via ADR, never silently:
...
| Prediction | ECE on temporal holdout **and** AUC | ECE ≤ 0.05 **and** AUC ≥ floor set after the logreg baseline (via ADR) |
...
| Engineering | one-command reproduce; CI green; every phase has ADR(s) + log | binary |
...
## 5. ADR-001 — Leakage protocol (core design constraint)
...
## 7. Tech stack (defaults — change only via ADR)

> The **LLM** and **Service/UI** bullets below supersede this PRD's earlier defaults (Anthropic API client + budget cap; Streamlit) per **ADR-002** and **ADR-003**, 2026-08-16.
...
- **Embeddings:** local `sentence-transformers` (e.g., `BAAI/bge-small-en-v1.5`) for zero-cost reproducibility. ADR required to switch to API embeddings.
...
- **Experiment tracking:** lightweight — JSONL runs + `docs/evals/*.md` reports. (MLflow only if this becomes painful; ADR it.)
...
├── PRD.md               # this file
├── docs/
│   ├── adr/             # ADR-001 committed in Phase 0; template included
│   │                    # ADR-002: LLM stages run as Claude Code workflows, no API client
│   ├── logs/            # weekly build logs: YYYY-WW.md
...
### Phase 0 — Scaffold
Repo structure above; tooling; GitHub Actions CI (lint + tests); ADR template + **ADR-001 committed**; build-log template; `Makefile` targets stubbed.
...
LLM extraction with structured JSON output, retries, caching; run over full corpus. Per-field precision/recall/F1 vs gold; error taxonomy in `docs/evals/extraction.md`. Implement scrub + **full ADR-001 validation suite**; wire leakage sentinel into CI on fixtures.
...
**DoD:** hybrid ≥ each single method on the judged set; indexes rebuild reproducibly via `make index`; and Recall@10 ≥ 0.70 on the judged set or ADR + gap analysis.

### Phase 5 — Overturn predictor
Baselines in strict order: majority class → logreg(structured fields) → logreg(TF-IDF **scrubbed** text — doubles as leakage sentinel) → LightGBM(structured + extracted features) → LLM few-shot with retrieved precedents (small eval subset only, for cost). **Temporal split** (train ≤ cutoff year, test after) to mimic deployment; **the cutoff year is fixed during Phase 1 EDA via ADR** (working proposal: train ≤ 2021, test 2022+) and not re-tuned afterwards. Report AUC/PR, reliability plot, ECE, SHAP top drivers. Write model card with selection-bias caveats per ADR-001.
...
**DoD:** citation-faithfulness meets the §3 target on the rubric set or ADR + gap analysis; 5 sample letters (good + failure cases) committed.
...
FastAPI: `/case` (intake), `/predict`, `/precedents`, `/draft_letter`, `/ask`. **React/Vite** flow built on the owner's design system (ADR-003): denial intake form → likelihood + drivers → precedent panel → letter editor → analytics tab. UI footer on every page: *"Drafts for professional review — not legal or medical advice. Estimates reflect external-review-stage likelihood only."* Final README (with demo GIF), architecture doc, cost report, model cards, and outlines for two blog posts (the leakage story; eval-first build).
**DoD:** `make demo` runs the full experience locally from a fresh clone.

## 10. Engineering standards (all phases)

- Every LLM stage emits a versioned artifact with its prompt hash and model (per §7 / ADR-002); artifacts summarized per phase in build logs. No budget cap — LLM work runs under the owner's Max subscription.
...
- Docs discipline: ADR for any PRD deviation; weekly log in `docs/logs/` covering decisions, dead ends, and costs — dead ends are content, not embarrassment.
...
| Risk | Mitigation |
|---|---|
| Residual verdict leakage inflates metrics | ADR-001 suite in CI; publish raw-vs-scrubbed sentinel gap; human audit |
...
| Targets in §3 prove unrealistic | revise via ADR with evidence — never silently |
...
- Gold-set public release → **yes: ReferenceIDs + annotations + join script only, never `Findings` text**, with a methodology note (2026-08-16; ADR-004).

**Still open:** blog platform; v1.1 priority (NY corpus vs payer-side QA flip vs hosted demo).

## 14. Kickoff instruction

> Claude Code: read this PRD fully, then execute **Phase 0** only. Produce the scaffold, ADR-001, CI, and templates. Stop at the DoD, write the first build-log entry (including anything in this PRD you found ambiguous), and wait for owner review before Phase 1.